...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        bail!("Chunk {} is missing", chunk_type)
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Png> {
        ...
        let header_bytes = chunks_iter.by_ref().take(8).copied().collect::<Vec<u8>>();

        if Self::STANDARD_HEADER != header_bytes.as_slice() {
            bail!("Invalid header bytes")
        }
        ...
        while chunks_iter.len() >= 12 {
            ...
            let chunk = Chunk::try_from(TryInto::<&[u8]>::try_into(
                chunks_iter
                    .by_ref()
                    .cloned()
